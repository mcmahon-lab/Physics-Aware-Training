"""
This module contains code faciliating the training of a physical neural network.
...
Functions:
--------
    get_cpu_state_dict(model):
        Save all state dict variables except the digital twins ones.
    set_cpu_state_dict(model, pretrained_dict):
...
class Module(nn.Module):
    ...
    train_epoch(device, train_loader, optimizer, epoch, log_interval = 1):
        One training epoch for a classifictaion task with NLL and lagrangian loss.
    test_epoch(device, test_loader):
