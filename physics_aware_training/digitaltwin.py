"""
This module contains the DigitalTwin class to handle data acquisition, 
training procedure and usage of the differentiable digital model for an
experiment.
"""
...
import torch
import os
from physics_aware_training.digital_twin_utils import *
import physics_aware_training.params_utils as pu

class DigitalTwin(torch.nn.Module):
    """ 
    A class to handle data acquisition, training procedure and usage of the differentiable 
    digital model for an experiment.
    ...
    def train_mean_dt(self, retake_data = False, Nx = 10000, **trainargs):
        ...
        **modelargs:
        ...
        if not os.path.exists('diff_digital_models'):
            os.makedirs('diff_digital_models')
    ...
    def take_dt_data(self, Nx = 10000):
        ...
        Nx : int, default: 10000
            Amount of training samples that is acquired if data is retaken.
        """
        if not os.path.exists('exp_io_data'):
            os.makedirs('exp_io_data')
    ...
    def get_dt_path(self):
        ...
        dt_path= os.path.join('diff_digital_models', filename)
    ...
    def get_dt_data_path(self):
        ...
        filename = ''.join(filename)
        dt_data_path= os.path.join('exp_io_data', filename)
