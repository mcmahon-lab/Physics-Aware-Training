'''
NOTE: this is a copy of DiscoverODEVariableParameters which has been modified for use in an example to illustrate how an imperfect calibration between experimental parameters and ODE parameters can be automatically overcome. 

It is NOT recommended to use this version for any other purpose - use the original file instead! 
'''

import torch
import torch.nn as nn
import lib.digital_twin_utils 
import lib.params_utils
...
class DiscoverODEVariableParameters(nn.Module):
    def __init__(self, 
        ...
        if len(parameterNunits)>0:
            ...
            self.parameterNet.weight.data = torch.diag(torch.ones(nparams)+0.01*torch.randn(nparams))
            self.parameterNet.bias.data = torch.zeros(nparams)+0.01*torch.randn(nparams)
        
        if NoCouplingInParameterNet:
            self.parameterNet = torch.nn.Parameter(torch.ones(nparams)+0.01*torch.randn(nparams))
            self.parameterNet_offset = torch.nn.Parameter(0.01*torch.randn(nparams))            
    ...
    def forward(self, x):
        ...
        inputs = lib.params_utils.inputsNormalized2Physical(inputs, self.input_range)
        ...
        outputs= lib.digital_twin_utils.outputsPhysical2Normalized(outputs, self.A, self.B)
